...
const CusTop = ({ logout }) => {
  ...
  const navigateto = () => {
    if (User === null) navigate(`/cprofile/1`);
    else navigate(`/cprofile/${User.id}`);
  };
  const gotoCart = () => {
    if (User === null) navigate(`/cart/1`);
    else navigate(`/cart/${User.id}`);
  };
    ...
    <Box>
      <Grid container>
        ...
        <Grid item xs={1} pt={2} pl={10}>
          <Link onClick={gotoCart} underline="none">
            <ShoppingCartIcon />
          </Link>
        </Grid>
        <Grid item xs={1} pt={1} pl={1}>
          <Button variant="outlined" onClick={logout} href="/">
            Logout
          </Button>
        </Grid>
